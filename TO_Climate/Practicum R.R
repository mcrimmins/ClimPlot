...
# Set date ranges - yearly data from 1895-present are available 
dateRangeStart="1895-01-01"
dateRangeEnd= "2022-12-31"

# generate date sequence -- keep with PRISM date
allDates<-seq(as.Date(dateRangeStart), as.Date(dateRangeEnd),by="year")
...
jsonQuery=paste0('{"bbox":"',ACISbbox,'","sdate":"',dateRangeStart,'","edate":"',dateRangeEnd,'","grid":"21","elems":"yly_maxt","meta":"ll,elev","output":"json"}') # or uid
...
#####
# plot some data - most recent year
...
writeRaster(gridStack, file="TO_PRISM_yearly_precip_1895_2022.tif")
#####

#####
# extract a yearly time series using TO boundary
yrmaxT <- t(extract(gridStack, TO, fun='mean', na.rm=TRUE, df=TRUE, weights = FALSE))
yrmaxT <- yrmaxT[2:nrow(yrmaxT),] # drop that first ID row
yrmaxT <- cbind.data.frame(allDates, yrmaxT)
head(yrmaxT)
#####
...
# plot a time series
library(ggplot2)
ggplot(yrmaxT, aes(allDates,yrmaxT))+
  geom_line()+
  ylab("Max Temperature (°F)")+
  xlab("year")+
  ggtitle("Yearly Max Temperature for Tohono O'odham Nation - PRISM 1895-2022")
#####
...
library("readxl")
Loggernet <- read_excel('C:/Users/Michelle/Documents/R/win-library/practicum/TO data.xlsx')
colnames(Loggernet)[1] ="alldates"
colnames(Loggernet)[2] ="yrmaxT"
head(Loggernet)

#####
# plot a time series
library(ggplot2)
ggplot(Loggernet, aes(alldates, yrmaxT)) +
  geom_line() +
  ylab("Max Temperature (°F)") +
  xlab("year") +
  ggtitle("Yearly Max Temperature for Tohono O'odham Nation - Loggernet 2003-2022")
#####
...
# plot a time series
library(ggplot2)
ggplot(yrmaxT, aes(x = alldates, y = yrmax, colour = "red")) +
  geom_line() +
  geom_line(data = Loggernet, aes(x = alldates, y = yrmax, colour = "green")) +
  ylab("Max Temperature (°F)") +
  xlab("year") +
  ggtitle("Yearly Max Temperature for Tohono O'odham Nation")
#####